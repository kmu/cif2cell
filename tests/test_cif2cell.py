...
class TestCif2Cell(unittest.TestCase):
    ...
    def test_vasp(self):
        # for path in glob.glob('cifs/*.cif'):
        p = subprocess.Popen(['./cif2cell', "-p", "vasp", "-f", 'cifs/Si.cif'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        stdout_data, stderr_data = p.communicate()
        print("OUT")
        print(stdout_data.decode())
    ...
    def test_castep(self):
        # for path in glob.glob('cifs/*.cif'):
        p = subprocess.Popen(['./cif2cell', "-p", "castep", "-f", 'cifs/Si.cif'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        stdout_data, stderr_data = p.communicate()
        print("OUT")
        print(stdout_data.decode())
